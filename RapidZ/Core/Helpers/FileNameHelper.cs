...
public static class FileNameHelper
{
    ...
    // Generate import filename based on input parameters
    public static string GenerateImportFileName(ImportInputs inputs)
    {
        var fileName = "Import";
        
        // Add first HS Code if available
        if (inputs.HSCodes.Any() && inputs.HSCodes.First() != "%")
        {
            fileName += "_" + inputs.HSCodes.First();
        }
        
        // Add first Product if available
        if (inputs.Products.Any() && inputs.Products.First() != "%")
        {
            fileName += "_" + CleanFileName(inputs.Products.First());
        }
        
        // Add first Importer if available
        if (inputs.Importers.Any() && inputs.Importers.First() != "%")
        {
            fileName += "_" + CleanFileName(inputs.Importers.First());
        }
        
        // Add first Foreign Country if available
        if (inputs.ForeignCountries.Any() && inputs.ForeignCountries.First() != "%")
        {
            fileName += "_" + CleanFileName(inputs.ForeignCountries.First());
        }
        
        // Add first Port if available
        if (inputs.Ports.Any() && inputs.Ports.First() != "%")
        {
            fileName += "_" + CleanFileName(inputs.Ports.First());
        }
        
        // Add month information
        string monthText = GetMonthRangeText(inputs.FromMonth, inputs.ToMonth);
        
        return $"{fileName}_{monthText}_IMP.xlsx";
    }
